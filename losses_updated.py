"""
=============================================================================
losses_updated.py — Complete Loss Functions for Phase A and Phase B
=============================================================================
"""

import torch
from config import (DEVICE, R_RAD, C0, C_ENV, T_COOL_STAR, FO_INV,
...
# ═════════════════════════════════════════════════════════════════════════════
# PHASE A — THERMAL LOSSES
# ═════════════════════════════════════════════════════════════════════════════

def loss_ic(model, x_s, y_s, t_s):
    """
    Initial condition - with hard IC enforcement, this should be ~0.
    Kept for monitoring and curriculum structure.
    """
    T_s, _, _ = model(x_s, y_s, t_s)
    return torch.mean(T_s**2)
...
def loss_bc_thermal_right(model, x_s, y_s, t_s):
    """Right edge: Radiation BC"""
    x_r = x_s.requires_grad_(True)
    T_s, _, _ = model(x_r, y_s, t_s)
    
    with torch.no_grad():
        props = get_props_star(x_r.detach(), y_s.detach(), T_s.detach())
    K_s = props['K_star']
    
    dT_dx = grad(T_s, x_r)
    r_rad = torch.tensor(float(R_RAD), dtype=torch.float32, device=DEVICE)
    c0 = torch.tensor(float(C0), dtype=torch.float32, device=DEVICE)
    c_env = torch.tensor(float(C_ENV), dtype=torch.float32, device=DEVICE)
    rad_rhs = r_rad * ((c0 + T_s)**4 - c_env**4)
    return torch.mean((-K_s * dT_dx - rad_rhs)**2)
...
# ═════════════════════════════════════════════════════════════════════════════
# PHASE B — ELASTIC LOSSES (with frozen thermal predictions)
# ═════════════════════════════════════════════════════════════════════════════

def loss_bc_elastic_top(model_elastic, model_thermal_frozen, x_s, y_s, t_s):
    """Top surface: traction-free"""
    x_r = x_s.requires_grad_(True)
    y_r = y_s.requires_grad_(True)
    
    with torch.no_grad():
        T_s, _, _ = model_thermal_frozen(x_r, y_r, t_s)
        props = get_props_star(x_r.detach(), y_r.detach(), T_s.detach())
    
    _, u_s, v_s = model_elastic(x_r, y_r, t_s)
...
def loss_bc_elastic_bottom(model_elastic, model_thermal_frozen, x_s, y_s, t_s):
...
def loss_bc_elastic_left(model_elastic, x_s, y_s, t_s):
    """Left symmetry: u* = 0"""
    _, u_s, _ = model_elastic(x_s, y_s, t_s)
    return torch.mean(u_s**2)


def loss_bc_elastic_right(model_elastic, model_thermal_frozen, x_s, y_s, t_s):
...
def loss_bc_elastic_inner(model_elastic, model_thermal_frozen, x_s, y_s, t_s, nx, ny):
...
def loss_elastic_pde(model_elastic, model_thermal_frozen, x_s, y_s, t_s):
...
class InterfaceNormalizer:
    """
    Tracks running mean for each interface loss component (L1, L2, L3, L4).
    Normalizes after warmup period to balance different scales.
    """
...
def loss_single_interface(model_elastic, model_thermal_frozen, 
                          xm, ym, xp, yp, t_if, nx, ny):
    """
    Interface conditions at single material boundary.
    Returns dict with L1, L2, L3, L4 (unnormalized).
    """
    # Minus side
    xm_r = xm.requires_grad_(True)
    ym_r = ym.requires_grad_(True)

    Tm, _, _ = model_thermal_frozen(xm_r, ym_r, t_if)
    with torch.no_grad():
        pm = get_props_star(xm_r.detach(), ym_r.detach(), Tm.detach())
    
    _, um, vm = model_elastic(xm_r, ym_r, t_if)
    ...
    Tp, _, _ = model_thermal_frozen(xp_r, yp_r, t_if)
    with torch.no_grad():
        pp = get_props_star(xp_r.detach(), yp_r.detach(), Tp.detach())
    
    _, up, vp = model_elastic(xp_r, yp_r, t_if)
...
def loss_all_interfaces(model_elastic, model_thermal_frozen, interfaces, normalizer):
    """
    Compute all interface losses with individual normalization.
    Returns total normalized loss + dict of components for logging.
    """
    ...
    for (xm, ym, xp, yp, t_if, nx, ny) in interfaces:
        loss_dict, (L1, L2, L3, L4) = loss_single_interface(
            model_elastic, model_thermal_frozen, xm, ym, xp, yp, t_if, nx, ny)
...
# ═════════════════════════════════════════════════════════════════════════════
# VALIDATION LOSS (Phase A and B)
# ═════════════════════════════════════════════════════════════════════════════

def compute_validation_loss_A(model, x_v, y_v, t_v):
    """Validation loss for Phase A — thermal PDE residual only"""
    ...
    return torch.mean(R_T**2).item()


def compute_validation_loss_B(model_elastic, model_thermal_frozen, x_v, y_v, t_v):
    """Validation loss for Phase B — elastic PDE residual only"""
    x_r = x_v.requires_grad_(True)
    y_r = y_v.requires_grad_(True)

    T_s, _, _ = model_thermal_frozen(x_r, y_r, t_v)
    with torch.no_grad():
        props = get_props_star(x_r.detach(), y_r.detach(), T_s.detach())

    _, u_s, v_s = model_elastic(x_r, y_r, t_v)
    ...
    return torch.mean(R_ex**2 + R_ey**2).item()
